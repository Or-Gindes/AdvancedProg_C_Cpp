...
if [ $number_of_docs -gt 1 ]; then
    ...
    result=`grep -oiwE "\b$first_char\w{$min_length,}" $read_file | grep -oi ':\w*' | grep -oi "\w*" | sort -f | uniq -ci | sort -n`
    [ ! -z "$result" ] && echo "$result"
else 
    read_file=${input_array[0]}
    result=`grep -oiwE "\b$first_char\w{$min_length,}" $read_file | sort -f | uniq -ci | sort -n`
    [ ! -z "$result" ] && echo "$result"
fi
